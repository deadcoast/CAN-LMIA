...
const MarkerClusterGroup: React.FC<MarkerClusterGroupProps> = ({ employers, onEmployerSelect }) => {
  ...
  useEffect(() => {
    ...
    // Clear existing markers - use a safer approach
    const layersToRemove: any[] = [];
    map.eachLayer((layer) => {
      // Check if layer has cluster-specific methods
      if (layer && typeof layer === 'object' && 'getLayers' in layer && 'addLayer' in layer) {
        layersToRemove.push(layer);
      }
    });
    layersToRemove.forEach(layer => map.removeLayer(layer));

    // Create cluster group using L.markerClusterGroup
    ...
    return () => {
      // Cleanup - remove all layers that might be cluster groups
      const layersToRemove: any[] = [];
      map.eachLayer((layer) => {
        if (layer && typeof layer === 'object' && 'getLayers' in layer && 'addLayer' in layer) {
          layersToRemove.push(layer);
